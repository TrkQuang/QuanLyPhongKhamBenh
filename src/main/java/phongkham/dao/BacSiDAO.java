...
public class BacSiDAO {
  ...
  public boolean deleteMaBacSi(String MaBacSi) {
    ...
    try (
      ...
      // Đã có lớp BUS kiểm tra trước khi xóa. Trường hợp này có thể do phát sinh dữ liệu liên quan đồng thời.
      return false;
    } catch (SQLException e) {
      e.printStackTrace();
    }
  ...
  public int countLichLamViecByMaBacSi(String maBacSi) {
  ...
  public int countLichKhamByMaBacSi(String maBacSi) {
    return countByDoctor(
  ...
  public int countHoSoBenhAnByMaBacSi(String maBacSi) {
    return countByDoctor(
  ...
  public int tongLienQuanByMaBacSi(String maBacSi) {
    return (
      countLichLamViecByMaBacSi(maBacSi) +
      countLichKhamByMaBacSi(maBacSi) +
      countHoSoBenhAnByMaBacSi(maBacSi)
    );
  }

  private int countByDoctor(String sql, String maBacSi) {
    try (
      Connection conn = DBConnection.getConnection();
      PreparedStatement ps = conn.prepareStatement(sql)
    ) {
  ...
  public boolean updateBacSi(BacSiDTO bs) {
    String sql =
      "UPDATE BacSi SET HoTen=?, ChuyenKhoa=?, SoDienThoai=?, Email=?, MaKhoa=? WHERE MaBacSi=?";
    try (
      Connection c = DBConnection.getConnection();
      PreparedStatement ps = c.prepareStatement(sql)
    ) {
