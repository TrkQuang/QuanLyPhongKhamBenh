...
public class KhoaPanel extends JPanel {
  ...
  private void renderList(ArrayList<KhoaDTO> data) {
    ...
    for (KhoaDTO khoa : data) {
      // Thống kê số bác sĩ
      long count = listBacSi
        .stream()
        .filter(
          bs ->
            bs.getMaKhoa() != null && bs.getMaKhoa().equals(khoa.getMaKhoa())
        )
        .count();
  ...
  private void xemDanhSachBacSi(KhoaDTO khoa) {
    List<BacSiDTO> ds = listBacSi
      .stream()
      .filter(
        bs -> bs.getMaKhoa() != null && bs.getMaKhoa().equals(khoa.getMaKhoa())
      )
      .collect(Collectors.toList());

    String content = ds.isEmpty()
      ? "Chưa có bác sĩ"
      : ds
          .stream()
          .map(b -> "- " + b.getHoTen())
  ...
  private void searchKhoa(String key) {
    ArrayList<KhoaDTO> filtered = listKhoa
      .stream()
      .filter(
        k ->
          k.getTenKhoa().toLowerCase().contains(key.toLowerCase()) ||
