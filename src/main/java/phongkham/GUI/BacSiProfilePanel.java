...
public class BacSiProfilePanel extends JPanel {
  ...
  private void handleUpdate() {
    int row = tableBacSi.getSelectedRow();
    if (row == -1 || !validateInput()) return;
    BacSiDTO updated = new BacSiDTO(
      lblMaBacSi.getText(),
      txtHoTen.getText().trim(),
      txtChuyenKhoa.getText().trim(),
      txtSDT.getText().trim(),
  ...
  private void handleDelete() {
    int row = tableBacSi.getSelectedRow();
    if (row == -1) return;
    String maBacSi = listBacSi.get(row).getMaBacSi();
    if (
  ...
  private void loadTableData() {
    ...
    for (BacSiDTO bs : listBacSi)
      tableModel.addRow(
        new Object[] { bs.getMaBacSi(), bs.getHoTen(), bs.getChuyenKhoa() }
      );
  }
  ...
  private String generateNextId() {
    ...
    for (BacSiDTO bs : listBacSi) {
      try {
        int num = Integer.parseInt(bs.getMaBacSi().replaceAll("[^0-9]", ""));
        if (num > max) max = num;
