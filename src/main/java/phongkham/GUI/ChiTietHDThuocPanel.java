...
public class ChiTietHDThuocPanel extends JPanel {
  ...
  public ChiTietHDThuocPanel(int maHoaDon, HoaDonThuocPanel parentPanel) {
  ...
  private JScrollPane createTablePanel() {
    String[] columns = {"Mã CTHD", "Mã thuốc", "Tên thuốc", "Đơn vị", "Số lượng", "Đơn giá", "Thành tiền"};
    ...
    private JPanel createBottomPanel() {
        ...
        if (hd != null) {
            ...
            if ("Chưa thanh toán".equals(trangThai)) {
                JButton btnThem = createStyledButton("Thêm", new Color(34, 197, 94), Color.WHITE);
                JButton btnSua = createStyledButton("Sửa", new Color(37, 99, 235), Color.WHITE);
                JButton btnXoa = createStyledButton("Xóa dòng", new Color(220, 38, 38), Color.WHITE);
            ...
            // Nút thanh toán
            if ("Chưa thanh toán".equals(trangThai)) {
                JButton btnThanhToan = createStyledButton("Thanh toán", new Color(34, 197, 94), Color.WHITE);
    ...
    private void themDong() {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Thêm chi tiết thuốc", true);
        ...
        // Lấy danh sách thuốc
        java.util.ArrayList<ThuocDTO> dsThuoc = new java.util.ArrayList<>(thuocBUS.list());
        JComboBox<String> cbThuoc = new JComboBox<>();
        for (ThuocDTO t : dsThuoc) {
            cbThuoc.addItem(t.getMaThuoc() + " - " + t.getTenThuoc());
        }

        JSpinner spinnerSoLuong = new JSpinner(new SpinnerNumberModel(1, 1, 1000, 1));
        ...
        btnLuu.addActionListener(e -> {
            ...
            CTHDThuocDTO cthd = new CTHDThuocDTO(maHoaDon, Integer.parseInt(maThuoc), soLuong, donGia);
    ...
    private void suaDong() {
        ...
        int maCTHD = (int) table.getValueAt(row, 0);
        CTHDThuocDTO cthd = bus.getDetailMedicine(maCTHD);

        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Sửa chi tiết thuốc", true);
        dialog.setLayout(new BorderLayout(10, 10));
        dialog.setSize(500, 250);
        dialog.setLocationRelativeTo(this);
        ...
        JSpinner spinnerSoLuong = new JSpinner(new SpinnerNumberModel(cthd.getSoLuong(), 1, 1000, 1));
        JTextField txtDonGia = new JTextField(String.valueOf(cthd.getDonGia()), 10);
        JTextField txtGhiChu = new JTextField(cthd.getGhiChu() != null ? cthd.getGhiChu() : "", 30);
    ...
    private void thanhToanHoaDon() {
        int confirm = JOptionPane.showConfirmDialog(this, "Xác nhận thanh toán hóa đơn này?", "Xác nhận", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
